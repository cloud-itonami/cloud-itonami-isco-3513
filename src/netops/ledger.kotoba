(ns netops.ledger
  "Append-only audit ledger for the ISCO-08 3513 community network-operations
  actor: entry construction, hash chaining, and verification.
  ...
  store's own docstring called it append-only, but nothing in the value made a
  dropped or reordered entry detectable: any prefix of the ledger, and any
  permutation of it, was indistinguishable from the real thing. 'Append-only'
  was a property of the code path, not of the artifact — and the code path is
  exactly what an audit is trying to check. For a record of which topology
  changes were approved for a network people depend on, that is the difference
  between an audit that can show which changes were approved and one that can
  only show which changes someone chose to leave in the file.

  Separately, the ledger could not answer the question this actor's
  human-approval interrupt exists to answer. Measured on the pre-change tree, a
  topology change applied through human sign-off and a routine draft committed
  automatically both produced `{:disposition :commit :record ...}` with no
  field distinguishing them. `commit-entry` therefore takes the approval
  provenance as part of the record, not as an afterthought.")
...
(defn entry
  ...
  [ledger m]
  (let [prev (if (seq ledger) (:ledger/hash (peek (vec ledger))) 0)
        seq-n (count ledger)
...
(defn verify
  ...
  A truncated ledger verifies as intact — a chain cannot detect that entries it
  never saw are missing. That is a real limit, not an oversight: detecting
  ...
  (let [v (vec ledger)]
    (loop [i 0 prev 0]
      (if (>= i (count v))
        {:ok? true :length (count v)}
        (let [e (nth v i)]
          (cond
            (not= (:ledger/seq e) i)
            {:ok? false :length (count v) :broken-at i :reason :seq-mismatch}

            (not= (:ledger/prev e) prev)
            {:ok? false :length (count v) :broken-at i :reason :prev-mismatch}

            (not= (:ledger/hash e)
                  (chain-hash prev (dissoc e :ledger/hash)))
            {:ok? false :length (count v) :broken-at i :reason :hash-mismatch}
...
(defn summary
  "One line per entry, for operators and for `netops.sim` output."
  [ledger]
  (apply str
         (interpose "\n"
                    (for [e ledger]
